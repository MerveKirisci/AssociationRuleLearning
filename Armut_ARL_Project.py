...
df = pd.read_csv("VBO9_Documents/datasets/armut_data.csv")
df.head()
df.describe().T
df.isnull().sum()
...
df.groupby(['SepetID', 'Service'])['Service'].count()
df.groupby(['SepetID', 'Service'])['Service'].count().unstack()
df.groupby(['SepetID', 'Service'])['Service'].count().unstack().fillna(0)
